...
const EKGChart = ({ rawData, filtData, connectionStatus, heatmapData, heatmapSignal }) => {
  ...
  useEffect(() => {
    ...
    const hh = setupCanvas(heatCanvas);
    ...
    const drawSignal = (ctx, data, w, h, color) => {
      ...
      const xStep = w / 999; // Fix to 1000 points (MAX_POINTS - 1) so it doesn't stretch initially
    ...
    const getColor = (v) => {
      ...
      const r = Math.floor(Math.max(0, Math.min(255, 255 * (1.5 - Math.abs(4 * v - 3)))));
      const g = Math.floor(Math.max(0, Math.min(255, 255 * (1.5 - Math.abs(4 * v - 2)))));
      const b = Math.floor(Math.max(0, Math.min(255, 255 * (1.5 - Math.abs(4 * v - 1)))));
      return `rgba(${r}, ${g}, ${b}, 0.5)`;
    };

    const drawHeatmap = (ctx, hData, hSignal, w, h) => {
      if (!hData || !hData.length || !hSignal || !hSignal.length) return;
      const xStep = w / (hData.length - 1);
      
      for (let i = 0; i < hData.length; i++) {
        const x = i * xStep;
        const val = hData[i];
        ctx.fillStyle = getColor(val);
        ctx.fillRect(Math.floor(x), 0, Math.ceil(xStep), h);
      }
      
      let min = Math.min(...hSignal);
      let max = Math.max(...hSignal);
      let range = max - min || 1;
      const yMin = min - range * 0.1;
      const yRange = (max + range * 0.1) - yMin;
      ...
      ctx.lineWidth = 2.0;
      ctx.lineJoin = 'round';
      for (let i = 0; i < hSignal.length; i++) {
        const x = i * xStep;
        const y = h - ((hSignal[i] - yMin) / yRange) * h;
        if (i === 0) ctx.moveTo(x, y);
        else ctx.lineTo(x, y);
      }
      ctx.stroke();
    ...
    const render = () => {
      ...
      hh.ctx.clearRect(0, 0, hh.w, hh.h);
      drawGrid(rr.ctx, rr.w, rr.h);
      drawGrid(ff.ctx, ff.w, ff.h);
      drawGrid(hh.ctx, hh.w, hh.h);
      drawSignal(rr.ctx, rawDataRef.current, rr.w, rr.h, '#0f172a');
      drawSignal(ff.ctx, filtDataRef.current, ff.w, ff.h, '#0ea5e9');
      drawHeatmap(hh.ctx, heatmapDataRef.current, heatmapSignalRef.current, hh.w, hh.h);
  ...
  return (
    <div className="ekg-chart-container glass-panel slide-in">
      ...
      <div className="chart-box">
        <p className="chart-label" style={{color:'#8b5cf6'}}>Yapay Zeka XAI Isı Haritası (Grad-CAM) - Modelin Odaklandığı Bölge</p>
        <canvas ref={heatCanvasRef} className="ecg-canvas"></canvas>
      </div>
    </div>
  );
};
